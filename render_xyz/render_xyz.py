...
app.use_app('egl')  # Set backend for headless rendering
...
class Renderer(app.Canvas):
    ...
    def __init__(self, size, cam):
        ...
        # (for qualitative results) Keep the back-face culling disabled because of objects which do not have
        # well-defined surface (e.g. the lamp from the lm dataset).
        self.disable_cull_face()
    ...
    # reference: https://github.com/fabi92/eccv18-rgb_pose_refinement/blob/master/rendering/renderer.py
    ...
    def compute_metrical_clip(self, pose, diameter):
        ...
        proj /= proj[2]
        cut = np.asarray([proj[1] - height // 2, proj[0] - width // 2, proj[1] + height // 2, proj[0] + width // 2],
                         dtype=int)
